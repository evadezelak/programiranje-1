...
def max_cheese(matrika):
    def pomozna(i, j):
        if i == (len(matrika)-1):
            ...
            dol = pomozna(i+1, j)
            desno = pomozna(i, j+1)
            return matrika[i][j] + max(dol, desno)
    return pomozna(0, 0)
...
def best_value_uniques(articles, max_w):
    ...
    def best_value(w): #za vsako od izbir, ki jo imamo, bomo naredili rekurzijo
        opcije = [] #seznam v katerega si shranjujemo vrednosti, ki jih dobimo
        ...
        for item in articles:
            ...
            if w - weight < 0:
                ...
                option = best_value(w - weight) + price
                opcije.append(option)
        
        if opcije:
            return max(opcije)
        else:
            return 0
    return best_value(max_w)
